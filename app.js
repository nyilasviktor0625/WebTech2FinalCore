...
app.locals.baseUrl = "http://89.134.136.66:3000"
...
app.post('/register', function (req, res) {
  ...
  connection.query(`SELECT * from admins where username="${usr.uname}"`, function(err, rows, fields) {
    if(rows.length <= 0){
      ...
      connection.query(`INSERT into admins(username, password) values("${usr.uname}","${pwwd}")`, function(err, rows, fields) {
        if(!err) {
          res.redirect('/login');
        } else {
...
app.get('/admin',(req,res)=> {
  res.render("admin/dashboard", {title: "Dash"})
})
//login script
  ...
  connection.query(`SELECT * from admins where username="${usr.uname}"`, function(err, rows, fields) {
    if (!err) {
      ...
      if(typeof rows[0] != 'undefined' && pwVerify(usr.pwd, rows[0].password)){
        ...
        res.render('/', {title: "Failed! Try again", failed: true});
      }
    }
    else
      throw err;
